...
# Uses a pair of 64-bit intergers to encode two (upper and lower) 8x8 binary arrays
...
def font_draw_8x8_character(vb: VBuffer, character: int, x: int, y: int, foreColor: int, backColor: int, fontROM: list[int] ) -> bool:

    # Check to see if we are drawing well out the range, that no partial drawing is possible
    if (x <= -8 or y <= -8 or x >= vb.get_dimensions()[0] or y >= vb.get_dimensions()[1]):
        return False
    ...
    if ((x + 8) >= vb.dimensions[0]):
        endX = vb.dimensions[0] - x

    if ((y + 8) >= vb.dimensions[1]):
        endY = vb.dimensions[1] - y

    # Parse the character from the font ROM
    targetCharacter = fontROM[character]

    characterData = [
        (0x00000000000000FF & targetCharacter) >> 0,
        (0x000000000000FF00 & targetCharacter) >> 8,
        (0x0000000000FF0000 & targetCharacter) >> 16,
        (0x00000000FF000000 & targetCharacter) >> 24,
        (0x000000FF00000000 & targetCharacter) >> 32,
        (0x0000FF0000000000 & targetCharacter) >> 40,
        (0x00FF000000000000 & targetCharacter) >> 48,
        (0xFF00000000000000 & targetCharacter) >> 56
        ]


    currentY = 0
    # Go through all the rows, starting at an offset and ending if partial drawing
    for rowData in characterData[startYoffset:endY]:
        ...
        currentX = 0
        for bit in rowBool[startXoffset: endX]:
            if bit:
                vb[x + startXoffset + currentX][y + startYoffset + currentY] = foreColor
            else:
                vb[x + startXoffset + currentX][y + startYoffset + currentY] = backColor
            currentX += 1
        currentY += 1

    return True

def font_draw_8x16_character(vb: VBuffer, character: int, x: int, y: int, foreColor: int, backColor: int, fontROM: list[int] ) -> bool:
    # Draw the upper 8x8 character
    font_draw_8x8_character(vb, character * 2, x, y, foreColor, backColor, fontROM)

    # Draw the lower 8x8 character
    font_draw_8x8_character(vb, character * 2 + 1, x, y + 8, foreColor, backColor, fontROM)


def font_draw_8x8_string(vb: VBuffer, string: str, x: int, y: int, foreColor: int, backColor: int, fontROM: list[int] ) -> bool:
    currentCharacterX = 0

    for character in string:


        font_draw_8x8_character(vb, ord(character), x + currentCharacterX, y, foreColor, backColor, fontROM)
        currentCharacterX += 8

def font_draw_8x16_string(vb: VBuffer, string: str, x: int, y: int, foreColor: int, backColor: int, fontROM: list[int] ) -> bool:
    currentCharacterX = 0

    for character in string:
        font_draw_8x16_character(vb, ord(character), x + currentCharacterX, y, foreColor, backColor, fontROM)
        currentCharacterX += 8
