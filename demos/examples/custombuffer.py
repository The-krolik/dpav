...
if WAVEXAMPLE != True:
    ...
    )  # we fetched a wav file's raw data, put it into a numpy array

test.set_audio_buffer(
    data
)  # either one of our numpy arrays are sent to audio.py for the playSound method

test.play_sound()  # notice playSound does not have any arguments. It's kind of implied that if you
# set your audio buffer to something custom, you don't supply any.

test.wait_for_sound_end()  # this makes the audio call blocking to prevent any premature stops
