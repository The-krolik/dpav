...
def playKey(pos, wsize, bKeys, wKeys):
    ...
    frequency = math.ceil(dpav.get_note_from_string(key.note, key.octave))
    mySound = dpav.Audio()
...
def drawWhiteKeys(buf, numKeys, keySize, keySpacing):
    ...
    for i in range(numKeys * 2 - 1):
        if drawkey:
            ...
            dpav.draw_rectangle(buf, 0xFFFFFF, start, end)
            wKeys.append(Key([start, end], "white", None, None))
            x += keySize
        else:
            start = (x, 0)
            end = (x + keySpacing, dim[1])
            dpav.draw_rectangle(buf, 0x000000, start, end)
            x += keySpacing
