...
class Audio(object):
    ...
    def getAudioDevice(self): # TODO: need audio buffer type for return annotation
        """
        Returns the audio buffer of the Audio class
        IN: None
        OUT: Returns a Numpy Array TODO: Type right?
    ...
    def setAudioDevice(self) -> None:
        ...
        """
        count = 0
        tests = sd.query_devices()
        for test in tests:
            print(count, test['name'])
            print("\n")
            count += 1
        count = int(input("Enter integer for device you want: "))
        test = tests[count]
        self.name = test['name']
    ...
    def playSound(self, inputFrequency, inputDuration)->None:
        ...
        pygame.mixer.pre_init(sampleRate, -bitNumber, 6, devicename=self.name)
        pygame.mixer.init()
